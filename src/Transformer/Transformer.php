...
namespace Xooxx\Api\Transformer;

use Xooxx\Api\Mapping\Mapper;
...
abstract class Transformer implements StrategyInterface
{
     ...
     */
    public function __construct(Mapper $mapper, $attributesCase = 'snake_case')
    {
    ...
    protected function noMappingGuard()
    {
        if (empty($this->mappings) || !is_array($this->mappings)) {
            throw new TransformerException('No mappings were found. Mappings are required by the transformer to work.');
        }
    }
    /**
    ...
    {
        $links = \array_filter([self::SELF_LINK => $this->getSelfUrl(), self::FIRST_LINK => $this->getFirstUrl(), self::LAST_LINK => $this->getLastUrl(), self::PREV_LINK => $this->getPrevUrl(), self::NEXT_LINK => $this->getNextUrl()]);
        return $links;
    }
    /**
     * @return string
    ...
    /**
     * @return \Xooxx\Api\Mapping\Mapping[]
     */
