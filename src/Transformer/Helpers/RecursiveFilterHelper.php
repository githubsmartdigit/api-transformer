...
final class RecursiveFilterHelper
{
     ...
     */
    protected static function deleteMatchedClassNotInFilterProperties(array &$mappings, array &$array, $typeKey, $type, array &$newArray)
    {
        if (\is_scalar($type) && $type === $typeKey) {
            $keepKeys = $mappings[$typeKey]->getFilterKeys();
            $idProperties = $mappings[$typeKey]->getIdProperties();
            $keepKeys = str_replace(array_values($mappings[$typeKey]->getAliasedProperties()), array_keys($mappings[$typeKey]->getAliasedProperties()), $keepKeys);
            if (!empty($keepKeys)) {
     ...
     */
    protected static function filterKeys(array &$mappings, array &$array, $typeKey, array &$newArray, array &$keepKeys, array &$idProperties)
    {
        foreach ($array as $key => &$value) {
            if (self::isPreservableKey($key, $keepKeys, $idProperties) || false === in_array($key, $mappings[$typeKey]->getProperties())) {
                $newArray[$key] = $value;
     ...
     * @return bool
     */
    protected static function isPreservableKey($key, array $keepKeys, array $idProperties)
    {
        return $key == Serializer::CLASS_IDENTIFIER_KEY || (\in_array($key, $keepKeys, true) || \in_array($key, $idProperties, true));
    }
}