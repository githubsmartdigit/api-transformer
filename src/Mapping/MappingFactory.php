...
use Xooxx\Api\Mappings\ApiMapping;
use Xooxx\Api\Mappings\HalMapping;
use Xooxx\Api\Mappings\JsonApiMapping;
use ReflectionClass;
class MappingFactory
{
    ...
    const REQUIRE_PROPERTIES_KEY = 'require_properties';
    ...
    public static function fromClass($className)
    {
        ...
        if (!class_exists($className, true)) {
            throw new MappingException(\sprintf('Provided class %s could not be loaded.', $className));
        }

        $instance = new $className();
        if (!in_array(ApiMapping::class, \class_implements($instance, true))) {
            throw new MappingException(\sprintf('Class %s must implement %s.', \ltrim($className, '\\'), ApiMapping::class));
        }
        /* @var HalMapping $instance */
        $mappedClass = [static::CLASS_KEY => $instance->getClass(), static::ALIAS_KEY => $instance->getAlias(), static::ALIASED_PROPERTIES_KEY => $instance->getAliasedProperties(), static::REQUIRE_PROPERTIES_KEY => $instance->getRequiredProperties(), static::HIDE_PROPERTIES_KEY => $instance->getHideProperties(), static::ID_PROPERTIES_KEY => $instance->getIdProperties(), static::URLS_KEY => $instance->getUrls()];
        if (\in_array(HalMapping::class, \class_implements($instance, true))) {
            $mappedClass[static::CURIES_KEY] = $instance->getCuries();
        }
        /* @var JsonApiMapping $instance */
    ...
    protected static function getClass(array &$mappedClass)
    {
        if (empty($mappedClass[static::CLASS_KEY])) {
            throw new MappingException('Could not find "class" property. This is required for class to be mapped');
        }
    ...
    protected static function getSelfUrl(array &$mappedClass)
    {
        if (empty($mappedClass[static::URLS_KEY][static::SELF_KEY])) {
            throw new MappingException('Could not find "self" property under "urls". This is required in order to make the resource to be reachable.');
        }
    ...
    {
        if (false === empty($mappedClass[static::REQUIRE_PROPERTIES_KEY])) {
            $mapping->setRequiredProperties($mappedClass[static::REQUIRE_PROPERTIES_KEY]);
            foreach ($mapping->getRequiredProperties() as $propertyName) {
                if (false === \in_array($propertyName, static::getClassProperties($className), true)) {
                    throw new MappingException(\sprintf('Could not require property %s in class %s because it does not exist.', $propertyName, $className));
                }
            }
        }
    }
